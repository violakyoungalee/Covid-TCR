...
#kmermaster <- kmermaster[c(1:50000),]
...
hit[intersect(which(volcano.pw$delta < 0), which(volcano.pw$pvalue < 0.05))] <- "Depleted"
klabel <- rep("kmer", dim(volcano.pw)[1])
klabel[intersect(which(volcano.pw$delta > 0), which(volcano.pw$pvalue < 0.05))] <- "Enriched"
klabel[intersect(which(volcano.pw$delta < 0), which(volcano.pw$pvalue < 0.002))] <- "Depleted"
klabel[intersect(which(volcano.pw$delta < -0.0003), which(volcano.pw$pvalue < 0.05))] <- "Depleted"
volcano.pw <- cbind(volcano.pw, hit = hit, klabel = klabel)
...
#volcano.pw <- volcano.pw[order(volcano.pw$sig, decreasing = T),]
write.table(volcano.pw, paste0("dex/su_cd8_volcano_",kmersize,"_",expid,".csv"), quote = F, row.names = F, col.names = T, sep = ",")
...
plt <- plt + labs(x = expression(Delta*" Mean"), y = "-log10( adj. p-value )")
...
plt <- plt + geom_point(data=subset(volcano.pw, hit == "Enriched"), size = 2, show.legend = F) 
plt <- plt + geom_text_repel(data=subset(volcano.pw, klabel == "Enriched"), mapping=aes(delta,sig,label=kmer), max.overlaps=1000, size = 8, box.padding = 0.2, point.padding = 0.6, show.legend = FALSE, seed = 17)
plt <- plt + geom_point(data=subset(volcano.pw, hit == "Depleted"), size = 2, show.legend = F) 
plt <- plt + geom_text_repel(data=subset(volcano.pw, klabel == "Depleted"), mapping=aes(delta,sig,label=kmer), max.overlaps=1000, size = 8, box.padding = 0.2, point.padding = 0.6, show.legend = FALSE, seed = 17)
plt <- plt + ggtitle(paste0("Volcano plot for ",expid," vs HD, ",kmersize,"-mers"))
pdf(paste0("dex/su_cd8_volcano_",kmersize,"_",expid,".pdf"), width = 10, height = 6)
plt
dev.off()
...
kmer <- kmermaster$`kmermaster[, 1]`
wtindex <- match(wtsamples, colnames(kmermaster))
expindex <- match(expsamples4, colnames(kmermaster))
wt <- as.data.frame(kmermaster)[,wtindex]
exp <- as.data.frame(kmermaster)[,expindex]
w.pw <- numeric()
diff.pw <- numeric()
...
#widx <- cbind(w.pw, diff.pw)

kmermastertemp1 <- kmermaster[order(w.pw),]
kmermastertemp1 <- kmermastertemp1[order(diff.pw[order(w.pw)]),]
kmermastertemp1 <- kmermastertemp1[c(1:20),]
kmermastertemp2 <- kmermaster[order(w.pw),]
kmermastertemp2 <- kmermastertemp2[order(diff.pw[order(w.pw)], decreasing = T),]
kmermastertemp2 <- kmermastertemp2[c(1:50),]
kmermaster <- rbind(kmermastertemp1, kmermastertemp2)
kmermaster <- as.data.frame(kmermaster)
kmermaster <- kmermaster[!duplicated(kmermaster), ]



####
kmermaster <- fread("su_cd8_4mers.csv")
kmermaster <- as.data.frame(kmermaster)
...
fwrite(kmermaster,"su_cd8_3mers.csv", quote = F)


kmermaster <- fread("su_cd8_3mers.csv")
...
qcpc <- qcpc + theme_bw() + theme(panel.border = element_rect(colour = "black", fill=NA, size = 0.8), axis.text=element_text(size=15), axis.title=element_text(size=20), axis.line = element_line(size = 0),  legend.title=element_text(size=0), legend.text=element_text(size=20), plot.title = element_text(size=22))
qcpc <- qcpc + labs(x = "PC1", y = "PC2") 
qcpc <- qcpc + scale_shape_manual(values=c(21, 2, 5)) + ggtitle("PCA Su cd8 dataset, 3-mers")
pdf("su_cd8_pca_3mer.pdf", height = 5, width = 8)
qcpc
dev.off()

pdf("su_cd8_pca_3mer_scree.pdf", height = 5, width = 8)
fviz_eig(pcoutput) + ggtitle("Scree plot of principal components for Su cd8 dataset, 3-mers")
dev.off()
...
pdf("su_cd8_3mer_volcano_moderate.pdf", width = 10, height = 6)
plt
dev.off()
...
pdf("su_cd8_3mer_volcano_mild.pdf", width = 10, height = 6)
plt
dev.off()
...
pdf("su_cd8_3mer_volcano_severe.pdf", width = 10, height = 6)
plt
dev.off()
...
fwrite(kmermaster,"su_cd8_4mers.csv", quote = F)
...
qcpc <- qcpc + theme_bw() + theme(panel.border = element_rect(colour = "black", fill=NA, size = 0.8), axis.text=element_text(size=15), axis.title=element_text(size=20), axis.line = element_line(size = 0),  legend.title=element_text(size=0), legend.text=element_text(size=20), plot.title = element_text(size=22))
qcpc <- qcpc + labs(x = "PC1", y = "PC2") 
qcpc <- qcpc + scale_shape_manual(values=c(21, 2, 5)) + ggtitle("PCA Su cd8 dataset, 4-mers")
pdf("su_cd8_pca_4mer.pdf", height = 5, width = 8)
qcpc
dev.off()

pdf("su_cd8_pca_4mer_scree.pdf", height = 5, width = 8)
fviz_eig(pcoutput) + ggtitle("Scree plot of principal components for Su cd8 dataset, 4-mers")
dev.off()
...
plt <- plt + geom_point(data=subset(volcano.pw, hit == "Depleted"), size = 3, show.legend = F) + geom_text_repel(data=subset(volcano.pw, hit == "Depleted"), mapping=aes(delta,sig,label=kmer), size = 5, box.padding = 0.2, point.padding = 0.6, show.legend = FALSE, seed = 17)
plt <- plt + ggtitle("Volcano plot for moderate vs HD")
pdf("su_cd8_4mer_volcano_moderate.pdf", width = 10, height = 6)
plt
dev.off()
...
plt <- plt + geom_point(data=subset(volcano.pw, hit == "Depleted"), size = 3, show.legend = F) + geom_text_repel(data=subset(volcano.pw, hit == "Depleted"), mapping=aes(delta,sig,label=kmer), size = 5, box.padding = 0.2, point.padding = 0.6, show.legend = FALSE, seed = 17)
plt <- plt + ggtitle("Volcano plot for mild vs HD")
pdf("su_cd8_4mer_volcano_mild.pdf", width = 10, height = 6)
plt
dev.off()
...
plt <- plt + geom_point(data=subset(volcano.pw, hit == "Depleted"), size = 3, show.legend = F) + geom_text_repel(data=subset(volcano.pw, hit == "Depleted"), mapping=aes(delta,sig,label=kmer), size = 5, box.padding = 0.2, point.padding = 0.6, show.legend = FALSE, seed = 17)
plt <- plt + ggtitle("Volcano plot for severe vs HD")
pdf("su_cd8_4mer_volcano_severe.pdf", width = 10, height = 6)
plt
dev.off()
...
pdf("../immunarch_outputs/immunarch_figs/geneUsage_immunarch_su_cd8_trbv.pdf", width = 20, height = 10)
...
pdf("../immunarch_outputs/immunarch_figs/geneUsage_immunarch_su_cd8_trbj.pdf", width = 20, height = 10)
print(vis(imm_gu, .by = "Disease status", .meta = immdata$meta))
print(vis(imm_gu, .by = "Disease status", .meta = immdata$meta, .plot = "box"))
dev.off()
...
pdf("../immunarch_outputs/immunarch_figs/repOverlap_immunarch_su_cd8.pdf", width = 50, height = 50)
print(vis(ov, "circos"))
dev.off()
...
pdf("cd8_stats_analyses/clonotype_stats_total_immunarch.pdf", width = 50, height = 20)
print(p1 + p2)
dev.off()
...
pdf("cd8_stats_analyses/distro_lengths.pdf", width = 150, height = 20)
print(p1)
dev.off()
pdf("cd8_stats_analyses/distro_abundances.pdf", width = 150, height = 20)
print(p2)
dev.off()
pdf("cd8_stats_analyses/distro_clonotypes.pdf", width = 150, height = 20)
print(p3)
dev.off()

p4 <- vis(exp_len, .by = "Disease status", .meta = immdata$meta)
p5 <- vis(exp_cnt, .by = "Disease status", .meta = immdata$meta)
pdf("cd8_stats_analyses/distro_lengths_dz.pdf", width = 150, height = 20)
print(p4)
dev.off()
pdf("cd8_stats_analyses/distro_abundances_dz.pdf", width = 150, height = 20)
print(p5)
dev.off()
...
#test <- fread("../gliph2/trial1/ref_cd88_v2.0.txt", header = F)
