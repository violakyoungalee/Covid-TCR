...
meta <- fwrite(meta_pool, "../vdj_datasets/subdata_clonal_prop/su_cd8/metadata.txt", quote = F, sep = "\t")
meta <- write.table(meta_pool$Sample, "../vdj_datasets/subdata_clonal_prop/su_cd8_sub.txt", quote = F, sep = "\t", row.names = F, col.names = F)
...
immdata <- repLoad("../vdj_datasets/subdata_clonal_prop/su_cd8/")
meta <- fread("../vdj_datasets/subdata_clonal_prop/su_cd8/metadata.txt")
...
pdf("barstack_top_su_cd8.pdf", width = 14, height = 10)
plt
dev.off()
...
pdf("barstack_rare_su_cd8.pdf", width = 14, height = 10)
plt
dev.off()
